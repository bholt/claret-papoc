...
```{r stress, fig.cap="Throughput of SortedSet microbenchmark.\\label{stress_tput}"}
d <- data(db("
...
d.u <- subset(d, nshards == 4 & nkeys == 10000 & alpha == 0.6 & grepl('update_heavy|read_heavy', mix))
ggplot(d.u, aes(x=nclients, y=throughput, group=cc, fill=cc, color=cc, linetype=cc))+
    ...
    expand_limits(y=0)+
    facet_wrap(~opmix)+
    theme_mine+theme(legend.position='top', legend.direction='horizontal', legend.title.align=1)+
    cc_scales(title='Concurrency\ncontrol:')
```

```{r throughput, fig.cap="Throughput on social network workloads.\\label{tput}"}
d <- data(db("
    select * from tapir where 
